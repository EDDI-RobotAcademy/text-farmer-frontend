<template>
    <div class="container">
        ...
        <div class="loginWrapper">
            <h2 class="login">로그인하기</h2>

            <button class="iconWrapper" @click="goToKakaoLogin">
                <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                ...
                <div class="snsLogin">카카오로 로그인하기</div>
            </button>

            <button class="iconWrapper" @click="snackbar = true">
                <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                ...
                <div class="snsLogin">네이버로 로그인하기</div>
            </button>
            ...
            <button class="iconWrapper" @click="snackbar = true">
                ...
                <div class="snsLogin">구글로 로그인하기</div>
            </button>
        </div>
...
export default {
    ...
    mounted() {
        function init() {
            ...
            var starLight = "";
            var countingStar = 250;
            var star = document.querySelector(".constellation");
            var windowWidth = window.innerWidth;
            var windowHeight = window.innerHeight;

            for (var i = 0; i < countingStar; i++) {
                starLight +=
                    "<span class='starLight " +
                    ...
                    "s; left: " +
                    getRandomArbitrary(0, windowWidth) +
                    "px; top: " +
                    getRandomArbitrary(0, windowHeight) +
                    "px;'></span>";
            }

            star.innerHTML = starLight;

            var periodOfFallingStar = 100;

            setTimeout(function () {
                fallingStarPath();
            }, periodOfFallingStar);

            function fallingStarPath() {
                setTimeout(fallingStarPath, periodOfFallingStar);
                periodOfFallingStar = getRandomArbitrary(5000, 10000);
                var fallingStar =
                    "<div class='fallingStar " +
                    style[getRandomArbitrary(0, 4)] +
                    "'></div>";
                document.getElementsByClassName("shootingStar")[0].innerHTML =
                    fallingStar;
                setTimeout(function () {
...
.fallingStar.style1 {
    animation-name: fallingStarStyle1;
}
.fallingStar.style2 {
    animation-name: fallingStarStyle2;
}
.fallingStar.style3 {
    animation-name: fallingStarStyle3;
}
.fallingStar.style4 {
    animation-name: fallingStarStyle4;
}
...
.iconWrapper:nth-of-type(1) {
    background-color: #fee500;
    box-shadow: 0 5px #ede9c1, 0 10px 15px rgba(255, 255, 255, 0.8);
}

.iconWrapper:nth-of-type(2) {
    background-color: #03c75a;
    box-shadow: 0 5px #9cb4d4, 0 10px 15px rgba(255, 255, 255, 0.8);
}

.iconWrapper:nth-of-type(3) {
    background-color: #ffffff;
    box-shadow: 0 5px #c5deda, 0 10px 15px rgba(255, 255, 255, 0.8);
}

.iconWrapper:active {
    box-shadow: 0 2px #0056b3, 0 4px 6px rgba(0, 0, 0, 0.2);
...
.iconWrapper:nth-of-type(3) > .snsLogin {
    margin-left: 1.65rem;
}
</style>
